...
def edit_account_details(request):
    form = CustomUserChangeForm(instance=request.user)
    return TemplateResponse(request, 'users/partials/edit_account_details.html', {'form': form, 'user': request.user})
...
def logout_view(request):
    logout(request)
    if request.headers.get('HX-Request'):
        return HttpResponse(headers={'HX-Redirect':reverse('users:login')})
    return redirect('shop:index')
