...
class CheckoutView(CartMixin, View):
    ...
    def get(self, request):
        ...
        context = {'form': form, 'cart': cart, 'cart_items': cart.items.select_related('product', 'product_size__size').order_by('-added_at'), 'total_price': total_price}
    ...
    def post(self, request):
        ...
        if not payment_provider or payment_provider not in ['stripe', 'heleket']:
            context = {'form': OrderForm(user=request.user),
                       'cart': cart,
                       'cart_items': cart.items.select_related('product', 'product_size__size').order_by('-added_at'),
                       'total_price': cart.sub_total,
                       'error_message': 'Please select your payment provider (Stripe or Heleket)'}
        ...
        form = OrderForm(user=request.user, data=form_data)
        if form.is_valid():
            print("Форма нормлаьная")
