...
from .forms import AddCartItemForm, UpdateCartItemForm
import json
...
class CartModalView(CartMixin, TemplateView):
    template_name = 'cart/cart_modal.html'
...
class AddCartItemView(CartMixin, View):
    @transaction.atomic
    def post(self, request, slug):
        cart = self.get_cart(request)
        product = get_object_or_404(Product, slug=slug)
        print(product)
        form = AddCartItemForm(request.POST, product=product)
        print(form.is_valid())
        ...
        if product_size.stock < quantity:
            return JsonResponse({'error': f'Only {product_size.stock} stock is available'}, status=400)
        ...
        if existing_item:
            ...
            if total_quantity > product_size.stock:
                return JsonResponse({
                    'error': f'Cannot add {quantity} items. Only {product_size.stock} stock is available'},status=400)
        ...
        if request.headers.get('HX-Request'):
            ...
            return JsonResponse({
                ...
                'message': f'Added {product.name}, {quantity} items.',
    ...
    def post(self, request, item_id):
        ...
        if quantity == 0:
            ...
            if quantity > cart_item.product_size.stock:
                return JsonResponse({
                    'error': f'Only {cart_item.product_size.stock} stock is available'
                })
